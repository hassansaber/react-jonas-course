...
export async function getCabinPrice(
  id: number
): Promise<Pick<CabinType, "regularPrice" | "discount"> | null> {
  const { data, error } = await supabase
    .from("cabins")
...
export async function getCountries(): Promise<CountryType[] | null> {
  try {
...
export async function createBookingcreateBooking(
...
export async function deleteBooking(id: number): Promise<BookingType | null> {
